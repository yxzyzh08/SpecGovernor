...
```
RD 阶段：
  客户端 1: RD Generator Agent（生成需求文档）
  客户端 2: RD Reviewer Agent（评审需求文档）
  客户端 1: 根据评审结果修订文档

PRD 阶段：
  客户端 3: PRD Generator Agent（基于 RD 生成产品文档）
...
| **G-4** | **支持中小型复杂项目** | 系统需能应对代码量在 100 万行以下的中型复杂项目。性能保证：影响分析 < 30 秒，模块级一致性检查 < 2 分钟，全项目检查 < 10 分钟（10 个模块并行）。 |
...
| **FR-1.2** | Generator Agent 的输入必须包含：(1) 上游文档内容（由编排器精准裁剪），(2) 生成提示词模板，(3) 项目上下文元数据。输出必须是结构化格式（Markdown + YAML Front Matter）。 | 文档生成 Agent 族, 流程编排器 |
| **FR-1.3** | Reviewer Agent 的输入必须包含：(1) 待评审文档，(2) 上游文档（对照），(3) 评审检查清单。输出必须是结构化的评审报告（JSON 格式），包含问题列表、严重程度、建议修改。 | 文档评审 Agent 族 |
| **FR-1.4** | 代码与测试 Agent 需能基于 DD 和 PRD 的上下文，生成满足规范的测试用例和代码片段。同样采用 Generator-Reviewer 对模式。 | 代码与测试 Agent 族 |

### **4.2 项目索引与依赖管理**

| ID | 需求描述 | 关键交互组件 |
| :---- | :---- | :---- |
| **FR-2.1** | 系统需提供**索引构建命令**（如 `specgov index:build`），用于扫描项目并生成结构化索引。索引内容包括：(1) 模块划分，(2) 文档章节映射，(3) 代码符号（API、类、函数），(4) 依赖关系图。 | 索引构建服务 |
...
| **FR-2.3** | 系统需支持**增量索引更新**：当文档或代码变更时（通过 Git diff 检测），只更新受影响部分的索引，避免全量重建。 | 索引更新服务 |

### **4.3 影响分析与一致性检查**

| ID | 需求描述 | 关键交互组件 |
| :---- | :---- | :---- |
| **FR-3.1** | 系统需提供**影响分析命令**（如 `specgov analyze:impact --changed=<file>`），基于依赖图快速定位受影响的下游文档和代码。响应时间 < 30 秒，无需调用 AI（纯图查询）。 | 影响分析服务 |
...
| **FR-3.4** | 一致性检查的实现策略：<br>(1) 基于索引定位相关依赖链<br>(2) 只加载依赖链涉及的文档和代码片段（通常 < 5000 行）<br>(3) 构造精准上下文发送给 AI Agent<br>(4) 输出结构化差异报告（JSON 格式） | 一致性守护 Agent, 流程编排器 |
...
| ID | 性能指标 | 验收标准 |
| :---- | :---- | :---- |
| **NFR-2.1** | **影响分析响应时间** | < 30 秒（基于索引的纯图查询，不调用 AI） |
...
| **NFR-2.5** | **索引构建时间** | < 20 分钟（100 万行代码项目，一次性操作） |
| **NFR-2.6** | **增量索引更新** | < 1 分钟（单个模块的索引更新） |
...
| **NFR-6.5** | **索引构建成本**：一次性操作，10-20 次 AI 调用 | < $1 |
...
| :---- | :---- | :---- | :---- |
| **1. 生成** | `specgov rd:generate` | (1) 编排器读取项目上下文<br>(2) 调用 RD Generator Agent 生成初稿<br>(3) 保存到 `docs/rd.md`<br>(4) 更新 `.specgov/state.json` | RD Generator Agent<br>流程编排器 |
...
| **4. 索引更新** | `specgov index:update --scope=rd` | (1) 提取 RD 的结构化元数据<br>(2) 更新依赖图中的 RD 节点<br>(3) 保存到 `.specgov/index/` | 索引更新服务 |
...
| 触发场景 | CLI 命令 | 执行流程 | 输出 |
| :---- | :---- | :---- | :---- |
| **超级个体修改 RD** | `specgov analyze:impact --changed=docs/rd.md` | (1) Git diff 识别变更内容<br>(2) 基于依赖图查询下游节点<br>(3) 返回受影响的 PRD/DD/TD/Code 列表 | JSON 格式的影响范围报告 |
| **超级个体决定更新下游** | `specgov prd:regenerate --based-on=rd` | (1) 读取变更后的 RD<br>(2) 重新生成 PRD<br>(3) 自动触发 PRD 评审 | 更新后的 PRD 文档 |

### **6.3 一致性检查流程**

| 检查场景 | CLI 命令 | 执行流程 | 性能要求 |
| :---- | :---- | :---- | :---- |
| **检查单个需求的一致性** | `specgov check:consistency --scope=RD-REQ-005` | (1) 基于依赖图定位依赖链<br>(2) 只加载相关文档和代码片段（< 5000 行）<br>(3) 调用一致性守护 Agent 深度分析<br>(4) 输出差异报告 | < 2 分钟 |
...
# 7. 修订 PRD 后，继续生成 DD
$ specgov dd:generate --based-on=prd
生成完成: docs/dd.md
...
```
核心思想：不检查所有代码，只检查相关代码

实现步骤：
1. 预先构建项目索引（一次性成本）
2. 基于依赖图进行精准定位
3. 只加载相关的文档和代码片段（通常 < 5000 行）
4. 将上下文控制在 AI 窗口范围内（< 20K tokens）
```
  ...
  "nodes": [
    {
      ...
      "location": "docs/rd.md#L42-L45",
      "keywords": ["OAuth2", "登录", "认证"]
    },
    {
      "id": "PRD-FEAT-012",
      "type": "feature",
      "summary": "实现OAuth2登录流程",
      "location": "docs/prd.md#L128-L156",
      "keywords": ["OAuth2", "第三方登录"]
    },
    {
      "id": "DD-API-008",
      "type": "api_design",
      "summary": "POST /auth/oauth2/callback",
      "location": "docs/dd.md#L234-L256",
      "signature": "POST /auth/oauth2/callback"
    },
    {
      "id": "CODE-API-008",
      "type": "api_impl",
      "summary": "AuthController.oauth2Callback",
      "location": "src/auth/auth.controller.ts#L89-L112",
      "signature": "async oauth2Callback(req: Request)"
    }
  ],
  "edges": [
...
```python
# 伪代码：索引构建逻辑（由 Claude Code 执行）

def build_index(project_path):
    # 1. 扫描项目结构，识别模块
    modules = scan_project_structure(project_path)

    # 2. 提取文档元数据
    rd_nodes = extract_requirements_from_md("docs/rd.md")
    prd_nodes = extract_features_from_md("docs/prd.md")
    dd_nodes = extract_designs_from_md("docs/dd.md")

    # 3. 提取代码符号（调用 AST 解析器或使用 Grep）
    code_nodes = extract_code_symbols(project_path)
    ...
    # 5. 构建依赖图
    dependency_graph = {
        "nodes": rd_nodes + prd_nodes + dd_nodes + code_nodes,
        "edges": edges
    }

    # 6. 持久化到 .specgov/index/
    save_json(dependency_graph, ".specgov/index/dependency-graph.json")
    save_json(modules, ".specgov/index/modules.json")
```
...
def check_consistency(scope):
    ...
    elif scope == "UserModule":
        dependency_chain = get_module_dependencies("UserModule")
    elif scope == "full":
        return parallel_check_all_modules()

    # 2. 加载依赖链涉及的内容（精准裁剪）
    context = ""
    for node in dependency_chain:
        content = load_content(node.location)  # 例如：docs/rd.md#L42-L45
        context += f"\n【{node.type} - {node.id}】\n{content}\n"
...
"""
    ...
    response = call_ai_agent(prompt)  # 调用 Claude Code / Gemini CLI
...
def check_full_project():
    # 1. 获取所有模块
    modules = load_modules(".specgov/index/modules.json")

    # 2. 生成并行检查任务
    tasks = []
    for module in modules:
        task = f"specgov check:consistency --scope={module.name}"
        tasks.append(task)

    # 3. 输出任务清单，提示超级个体并行执行
    print("请在多个 AI 客户端中并行执行以下任务：")
    for i, task in enumerate(tasks):
        print(f"客户端 {i+1}: {task}")

    # 4. 等待所有任务完成，汇总报告
    reports = wait_for_all_tasks(tasks)
    return merge_reports(reports)

# 性能：10 个模块 × 2 分钟 / 10 客户端 = 2 分钟（并行）
# 成本：10 个模块 × $0.05 = $0.50
```
...
```
需求：RD-REQ-005（OAuth2 登录）

依赖链：RD-REQ-005 → PRD-FEAT-012 → DD-API-008 → CODE-API-008

加载内容：
- docs/rd.md 第 42-45 行（4 行）
- docs/prd.md 第 128-156 行（29 行）
- docs/dd.md 第 234-256 行（23 行）
- src/auth/auth.controller.ts 第 89-112 行（24 行）
...
# Agent 配置（可为不同 Agent 指定不同后端）
agents:
  rd-generator:
    backend: claude-code
  rd-reviewer:
    backend: gemini-cli  # 使用不同的 AI 避免偏差
  prd-generator:
    backend: claude-code
```
...
本需求文档定义了一个面向**超级个体**的 AI 增强型研发流程治理系统，核心特点包括：

1. ✅ **生成-评审对模式**：每个阶段都有独立的 Generator 和 Reviewer Agent，提高质量
2. ✅ **索引驱动的一致性检查**：通过预先构建的依赖图，支持 100 万行代码项目的高效检查
3. ✅ **纯 CLI 架构**：无需后台服务，部署简单，适合超级个体使用
4. ✅ **成本与性能可控**：通过精准的上下文裁剪，控制 AI 调用成本和响应时间
...
3. **原型验证**：实现核心流程（索引构建 + 影响分析 + 简单一致性检查）验证可行性
4. **Agent 提示词设计**：为 Generator 和 Reviewer Agent 设计高质量的提示词模板
